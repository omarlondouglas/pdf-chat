...
export function HeroWave({
  ...
  subtitle = "Faça perguntas sobre o documento carregado e obtenha respostas instantâneas.",
  ...
  const basePlaceholder = "Pergunte sobre";
  ...
  useEffect(() => {
    ...
    function setupQuickSetters() {
      const u = (instancedBars!.material as THREE.ShaderMaterial).uniforms;
    ...
    function buildKeyframeTweens(
      target: typeof wave1,
      keyframes: Array<{ time: number; gain: number; frequency: number; waveLength: number }>
    ) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <h1 className="text-white text-2xl sm:text-4xl font-semibold tracking-tight drop-shadow-[0_1px_8px_rgba(31,61,188,0.25)]">
            {title}
          </h1>
          <p className="text-gray-300/80 mt-2 sm:mt-3 text-xs sm:text-sm max-w-xl mx-auto">
            {subtitle}
          </p>
          <form
            ...
            <div className="relative w-full sm:w-[720px]">
              <div className="relative rounded-2xl p-[2px] shadow-[0_1px_2px_0_rgba(0,0,0,0.06)] bg-gradient-to-br from-white/10 via-white/5 to-black/20">
                <textarea
                  ...
                  rows={5}
                  disabled={loading}
                  className="w-full h-32 sm:h-36 resize-none rounded-2xl bg-[rgba(15,15,20,0.55)] border border-white/10 text-white placeholder:text-white/40 outline-none focus:ring-2 focus:ring-[#1f3dbc]/40 focus:border-[#1f3dbc]/40 backdrop-blur-md px-4 py-4 pr-16 disabled:opacity-60"
                />
              </div>
              <button
                type="submit"
